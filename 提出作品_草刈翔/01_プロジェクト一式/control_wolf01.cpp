...
//マクロ定義
//*****************************************************************************
#define WOLF01_GRAVITY				(0.6f)	// 重力の大きさ
#define WOLF01_MAX_GRAVITY			(15.0f)	// 重力の最大値
#define WOLF01_MOVE_SPEED			(0.8f)	// 移動量の基準値
#define WOLF01_INTERIA_SUBTRACTION	(0.86f)	// 地上での慣性の減算値
#define WOLF01_MOVE_STOP_COUNT		(0.02f)	// プレイヤーの移動量を0にする時の移動量の値
#define MAX_WOLF01_ATTACK_INTERVAL	(75)	// 基本の攻撃間隔
#define MAX_WOLF01_MOVE_RANGE		(2000)	// プレイヤーを追尾する範囲

//=============================================================================
// コンストラクタ
//=============================================================================
CControlWolf01::CControlWolf01()
{
	// 変数のクリア
...
// 初期化処理
//=============================================================================
HRESULT CControlWolf01::Init(void)
{
	// 変数の初期化
	m_move = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	m_moveOld = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	...
	m_fNumRot = 0.0f;
	m_nMoveCount = 0;
	m_nAtkInterval = MAX_WOLF01_ATTACK_INTERVAL;
	m_fSpeed = WOLF01_MOVE_SPEED;
	m_bRotate = false;
	m_bAttack = false;
	m_nAtkCount = 0;
	m_fAtkBeginEffect = 0;
...
// 終了処理
//=============================================================================
void CControlWolf01::Uninit(void)
{
...
// 更新処理
//=============================================================================
void CControlWolf01::Update(CScene *pScene)
{
	...
	// 重力
	m_move.y -= WOLF01_GRAVITY;

	// 重力が強くなりすぎたら重力の最大値で固定しておく
	if (m_move.y <= -WOLF01_MAX_GRAVITY)
	{
		m_move.y = -WOLF01_MAX_GRAVITY;
	}
	...
	if (pMotion != NULL)
	{
		...
		{
			// 移動量が0かつ、今が移動モーションかつ、モーションをつなげていなかったら
...
// 生成処理
//=============================================================================
CControlWolf01 *CControlWolf01::Create(void)
{
	// クラスの生成
	CControlWolf01 *pControlEnemy = NULL;
	pControlEnemy = new CControlWolf01;
...
// 移動処理
//=============================================================================
void CControlWolf01::Move(CEnemy *pEnemy)
{
	m_bMove = false;
		...
		while (pObject != NULL)
		{
			...
			if (pObject->GetObjType() == CScene::OBJECT_TYPE_PLAYER)
			{
				...
				if (pPlayer->GetDeath() == false)
				{
					...
					if (fRadius <= MAX_WOLF01_MOVE_RANGE && fRadius >= fStopRadius)
					{
						// 移動している状態にする
						m_bMove = true;
					...
					// プレイヤーを追尾する範囲外なら
					if (fRadius > MAX_WOLF01_MOVE_RANGE)
					{
						// 目的の向きを設定
						m_fObjectiveRot = fRot + D3DX_PI / 2.0f;
						// 回転の慣性をオンにする
...
// 攻撃処理
//=============================================================================
void CControlWolf01::Attack(CEnemy *pEnemy)
{
	// モーション取得処理
	CMotion *pMotion = pEnemy->GetMotion();
	...
	{
		if (m_nAttackWaitTime >= MAX_WOLF01_ATTACK_INTERVAL)
		{
			m_bAttack = true;
			...
			m_nAtkInterval = (rand() % 40 - 20) + MAX_WOLF01_ATTACK_INTERVAL;
		}
	}
	...
	if (pMotion->GetMotion() == 2)
	{
		...
		{
			if (m_nAtkCount >= 8 && m_nAtkCount <= 16)
			{
				// エネミーの向きを取得し、直進させる
...
// 攻撃時などのコリジョン生成処理
//=============================================================================
void CControlWolf01::DoingAttack(CEnemy *pEnemy)
{
	// モーション取得処理
	CMotion *pMotion = pEnemy->GetMotion();
	...
	{
		// エフェクトの開始フレームを設定
		m_fAtkBeginEffect = pDataAtk->GetAttackData(CDataAttack::ATTACK_TYPE_WOLF01, CDataAttack::ATTACK_DATA_TYPE_BEGIN_EFFECT);

		// エフェクトの持続時間を設定
		m_fAtkEffectCount = pDataAtk->GetAttackData(CDataAttack::ATTACK_TYPE_WOLF01, CDataAttack::ATTACK_DATA_TYPE_TIME_EFFECT);
		...
		if (m_nAtkCount == 22)
		{
			...
			CSound *pSound = CManager::GetSound();
			pSound->Play(19);
			pSound->ControllVoice(19, 0.6f);
		}

		// 特定のフレームになったらコリジョンをつける
		if (m_nAtkCount == pDataAtk->GetAttackData(CDataAttack::ATTACK_TYPE_WOLF01, CDataAttack::ATTACK_DATA_TYPE_BEGIN_COLLISION))
		{
			// エネミーの口にあたるモデルの位置・情報を取得
			D3DXVECTOR3 modelPos = pEnemy->GetModelPos(2);
			CModel *pModel = pEnemy->GetModel(2);

			// 口にコリジョン(敵攻撃判定)をつけ、親子関係を持たせる
			m_pCollisionS = CCollisionSphere::Create(D3DXVECTOR3(modelPos.x, modelPos.y, modelPos.z), 100.0f, CCollisionSphere::COLLISION_S_TYPE::COLLISION_S_TYPE_ENEMY_ATTACK, pDataAtk->GetAttackData(CDataAttack::ATTACK_TYPE_WOLF01, CDataAttack::ATTACK_DATA_TYPE_TIME_COLLISION));
			...
			// 倍率をそのまま攻撃力として設定
			m_pCollisionS->SetDamageValue(pDataAtk->GetAttackData(CDataAttack::ATTACK_TYPE_WOLF01, CDataAttack::ATTACK_DATA_TYPE_DAMAGE));
		}
	}
	...
	if (m_bAttack == true && pMotion->GetNextMotion() == pMotion->GetMotion())
	{
		...
		if (m_nAtkCount >= m_fAtkBeginEffect && m_nAtkCount <= m_fAtkBeginEffect + m_fAtkEffectCount)
		{
			...
			// 軌跡にエフェクトをつける
			CEffectB::Create(D3DXVECTOR3(modelPos.x - 8.0f, modelPos.y + 9.0f, modelPos.z - 17.0f), D3DXVECTOR3(25.0f, 25.0f, 0.0f),
				D3DCOLOR_RGBA(60, 100, 160, 255), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 35, CEffectB::EFFECT_TYPE_SMALL, pModel);
			CEffectB::Create(D3DXVECTOR3(modelPos.x + 8.0f, modelPos.y + 9.0f, modelPos.z - 17.0f), D3DXVECTOR3(25.0f, 25.0f, 0.0f),
				D3DCOLOR_RGBA(60, 100, 160, 255), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 35, CEffectB::EFFECT_TYPE_SMALL, pModel);
		}
		// カウントを増やす
...
// 移動の慣性についての処理
//=============================================================================
void CControlWolf01::MoveInteria(CEnemy *pEnemy)
{
	// 慣性の減算
	m_move.z *= WOLF01_INTERIA_SUBTRACTION;
	m_move.x *= WOLF01_INTERIA_SUBTRACTION;

	// 移動量が既定の値になったら0にする
	if (m_move.x <= WOLF01_MOVE_STOP_COUNT && m_move.x >= -WOLF01_MOVE_STOP_COUNT)
	{
		m_move.x = 0.0f;
	}
	if (m_move.z <= WOLF01_MOVE_STOP_COUNT && m_move.z >= -WOLF01_MOVE_STOP_COUNT)
	{
		m_move.z = 0.0f;
	}
}
...
// 回転処理
//=============================================================================
void CControlWolf01::Rotate(CEnemy *pEnemy)
{
	// プレイヤーの向きを取得
