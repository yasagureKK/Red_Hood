...
void CDataStage::LoadStage(int nNum)
{
	...
	if (pFile != NULL)
	{
		int nMaxField = 0, nMaxWall = 0, nMaxModel = 0, nMaxEnemy = 0, nMaxEnemyBox = 0;
		int nCntField = 0, nCntWall = 0, nCntModel = 0, nCntEnemy = 0, nCntEnemyBox = 0;
		...
		while (1)
		{
			...
			if (strcmp(&cString[0], "NUM_ENEMY_BOX") == 0)
			{
				fscanf(pFile, "%s%d", cString, &nMaxEnemyBox);
			}
			...
			/* エネミーボックスの配置 */
			/**************************/
			if (strcmp(&cString[0], "ENEMYBOXSET") == 0 && nCntEnemyBox < nMaxEnemyBox)
			{
				int nType = 0;
				...
				while (1)
				{
					...
					if (strcmp(&cString[0], "END_ENEMYBOXSET") == 0)
					{
						break;
					}
				}
				...
				// エネミーボックスの生成
				CEnemyBox::Create(pos, size, nType);
				nCntEnemyBox++;
			}
