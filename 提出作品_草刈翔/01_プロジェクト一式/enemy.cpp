...
HRESULT CEnemy::Init(D3DXVECTOR3 pos, D3DXVECTOR3 size, float fLife)
{
	...
	case ENEMY_TYPE_WOLF_0:
		...
		m_pMotion = CMotionWolf00::Create(this);
		m_nScore = 200;
	...
	case ENEMY_TYPE_WOLF_1:
		...
		m_pMotion = CMotionWolf01::Create(this);
		m_nScore = 300;
		// 体にあたるモデルの位置や情報を取得し、親子付け
		modelPos = GetModelPos(0);
		pModel = GetModel(0);
	...
	case ENEMY_TYPE_BOSS:
		...
		m_pMotion = CMotionBoss::Create(this);
		m_nScore = 3000;
	...
	else
	{
		...
		if (m_type == ENEMY_TYPE_WOLF_0)
		{
			fOffSet = m_size.y / 1.5f;
		}
		else if (m_type == ENEMY_TYPE_WOLF_1)
		{
			fOffSet = m_size.y / 1.1f;
		}
		// ライフと最大ライフを設定
		m_pLifeBill = CLifeBillboard::Create(D3DXVECTOR3(m_pos.x, m_pos.y, m_pos.z), fOffSet, 130.0f, 10.0f);
	...
	while (pObject != NULL)
	{
		...
		if (pObject->GetObjType() == CScene::OBJECT_TYPE_ENEMY)
		{
			...
			if (pObject != this)
			{
				...
				float fLifeEne = pObjEnemy->GetLife();			// 対象の敵のライフを取得
				...
				// 距離がエネミーのサイズより小さいかつ、自身のライフが残っているなら
				if (fDistance <= pEnemy->m_size.x && (pEnemy->GetLife() > 0.0f && fLifeEne > 0.0f))
				{
...
// 倒された時の処理
//=============================================================================
void CEnemy::KnockDown(ENEMY_TYPE type)
{
	if (m_bKnockDown == false)
	{
		// プレイヤーに経験値を渡す
...
void CEnemy::CanControl(CEnemy *pEnemy)
{
	...
	while (pObject != NULL)
	{
		...
		if (pObject->GetObjType() == CScene::OBJECT_TYPE_PLAYER)
		{
			...
			float fHeight = enePos.y - plyPos.y;

			// プレイヤーと敵との高さの差が3000.0f以上あるなら
			if (fHeight >= 3000.0f)
			{
				// 動作させない
				m_bCanControl = false;
