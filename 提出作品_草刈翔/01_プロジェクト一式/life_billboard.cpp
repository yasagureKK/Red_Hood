...
{
	m_pos = D3DXVECTOR3(pos.x, pos.y + m_fOffset, pos.z);
	m_size = size;
	m_fObjectiveSize = 0.0f;
	m_fMaxSize = m_size.x;
	...
	// ライフ(枠)
	m_apBillboard[0] = CBillboard::Create(m_pos, D3DXVECTOR3(size.x + 15.0f, size.y + 5.0f, size.z), 5, false);

	// ライフ(ゲージ)
	m_apBillboard[1] = CBillboard::Create(m_pos, m_size, 6, false);

	//  ライフ(空部分)
	m_apBillboard[2] = CBillboard::Create(m_pos, m_size, 7, false);
...
void CLifeBillboard::Update(void)
{
	for (int nCntLife = 0; nCntLife < MAX_LIFE_BILLBOARD; nCntLife++)
	{
		if (m_apBillboard[nCntLife] != NULL)
		{
			...
			D3DXVECTOR3 newPos = D3DXVECTOR3(m_pos.x, m_pos.y + m_fOffset, m_pos.z);
...
// 生成処理
//=============================================================================
CLifeBillboard *CLifeBillboard::Create(D3DXVECTOR3 pos, float fOffset, float fSizeX, float fSizeY)
{
	// クラスの生成
